...
def feature_matrix(bamfilepath, outputpath = '', max_worker = 8, vcfpath = '', window_size = 200, maxread = 18, block = 1e7, includecontig = [], genotype = False):
    if(outputpath == ''):
        if('/' in bamfilepath):
            outputpath = bamfilepath.split('/')[-1][:-4]
        else:
            outputpath = bamfilepath[:-4]
